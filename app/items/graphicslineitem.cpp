...
GraphicsLineItem::GraphicsLineItem(QGraphicsItem *parent)
  ...
  for (Handles::iterator it = m_handles.begin(); it != hend; ++it) delete (*it);
  m_handles.clear();
}
...
void GraphicsLineItem::addPoint(const QPointF &point) {
  ...
  SizeHandleRect *shr =
      new SizeHandleRect(this, dir + Left, dir == 1 ? false : true);
...
void GraphicsLineItem::stretch(int handle, double sx, double sy,
...
void GraphicsLineItem::paint(QPainter *painter,
  ...
  Q_UNUSED(widget);
  painter->setPen(pen());
  if (m_points.size() > 1) painter->drawLine(m_points.at(0), m_points.at(1));
}
