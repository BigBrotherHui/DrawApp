...
QPainterPath GraphicsPolygonItem::shape() const {
  ...
  path.closeSubpath();
  QRectF r = qt_graphicsItem_shapeFromPath(path, pen()).boundingRect();
  QPainterPath p;
  p.addRect(r);
  return p;
}
...
void GraphicsPolygonItem::stretch(int handle, double sx, double sy,
...
void GraphicsPolygonItem::endPoint(const QPointF &point) {
  ...
  if (nPoints > 2 &&
      (m_points[nPoints - 1] == m_points[nPoints - 2] ||
...
void GraphicsPolygonItem::paint(QPainter *painter,
  ...
  //  QColor c = brushColor();
  //  QLinearGradient result(boundingRect().topLeft(),
  //  boundingRect().topRight()); result.setColorAt(0, c.dark(150));
  ...
  for (int i = 0; i < m_points.size(); i++) {
    if (m_points.size() > i + 1) {
      path.lineTo(QPoint(m_points[i + 1].x(), m_points[i].y()));
      path.lineTo(QPoint(m_points[i + 1].x(), m_points[i + 1].y()));
    }
  }
  if (closePath) {
    path.lineTo(m_points[0].x(), m_points[m_points.size() - 1].y());
