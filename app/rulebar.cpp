...
QtRuleBar::QtRuleBar(Qt::Orientation direction, QGraphicsView *view,
                     QWidget *parent)
    : QWidget(parent), m_view(view), m_faceColor(0xFF, 0xFF, 0xFF) {
...
void QtRuleBar::drawTicker(QPainter *painter) {
  ...
  SPRulerMetric ruler_metric =
      ruler_metric_general; /* The metric to use for this unit system */
  ...
  for (scale = 0; scale < sizeof(ruler_metric.ruler_scale) /
                              sizeof(ruler_metric.ruler_scale[0]);
       scale++)
  ...
  for (i = sizeof(ruler_metric.subdivide) / sizeof(ruler_metric.subdivide[0]) -
           1;
       i >= 0; i--) {
    double subd_incr;
    if (scale == 1 && i == 1)
    ...
    for (cur = start; cur <= end; cur += subd_incr) {
      ...
      double label_spacing_px =
          fabs(increment * (double)ruler_metric.ruler_scale[scale] /
...
QtCornerBox::QtCornerBox(QWidget *parent) : QWidget(parent) {}
